...
def _compute_crop_shape(image_height, image_width, aspect_ratio, crop_proportion):
    ...
    def _image_wider_than_requested_aspect_ratio():
        crop_height = tf.cast(tf.rint(crop_proportion * image_height_float), tf.int32)
        crop_width = tf.cast(
...
def preprocess_for_train(
    image, height, width, color_distort=True, crop=True, flip=True
):
...
if __name__ == "main":
    ...
    with tf.Session() as sess:
        ...
        for i, image_path in tqdm(enumerate(image_paths)):
            ...
            key = module(inputs=image_tensor, signature="default", as_dict=True)[
                "final_avg_pool"
            ]
            key = key.eval().ravel()
            embdding.append(key)

    data_features = {"embeddings": embdding}
